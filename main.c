...
int main(int argc, char **argv)
{
	...
	source = argv[2];
	out = "Arch.a";
	Meta = open("Metadata", O_RDWR | O_CREAT, S_IRUSR | S_IWUSR);
	if (Meta == -1)
		exit(-2);
	...
	if (strcmp(action, "-p") == 0) {
		...
		output = open(out, O_WRONLY | O_CREAT, S_IRUSR | S_IWUSR);
		if (output == -1) {
			close(Meta);
			remove("Metadata");
			exit(-3);
...
int packfile(struct dirent *entry, struct stat statbuf, int output, int Meta, int depth)
{
	...
	memset(md, 0, sizeof(unsigned int)*3);
